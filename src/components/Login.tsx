...
function Login() {
    ...
    return (
        ...
        <section className="h-[calc(100vh-35vh)] w-[calc(100dvw-70dvw)] absolute top-[calc(100dvh-80dvh)] left-[calc(100dvw-65dvw)] flex justify-center items-center">
            <div className={`${ToggleLogin ? 'opacity-100 z-50' : 'rotate-6 z-40 animate-shuffle sm:left-0'}
                 duration-1000 w-[calc(100dvh-60dvh)] h-full p-6 shadow-lg  flex flex-col gap-10 absolute bg-white`}>
                ...
                <p className="text-sm text-gray-500">Already have an account? <span className="text-[#fca223] cursor-pointer" onClick={() => setToggleLogin(!ToggleLogin)}>Sign In</span></p>
            </div>
            <div className={`${ToggleLogin ? 'rotate-6 z-40 animate-shuffle sm:right-0' : ' opacity-100 z-50'}
                ...
                <p className="text-sm text-gray-500">Don't have an account? <span className="text-[#fca223] cursor-pointer" onClick={() => setToggleLogin(!ToggleLogin)}>Sign Up</span></p>
            </div>
        </section>
    )
}
